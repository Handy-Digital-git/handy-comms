...
"[project]/src/app/api/notifications/dispatch/route.ts [app-route] (ecmascript)", ((__turbopack_context__) => {
"use strict";
...
async function POST(req) {
    try {
        ...
        const { data: recipients, error } = await supabase.from("notification_recipients").select("name, phone, email, role, days_worked, enabled").eq("enabled", true).eq("role", requiredRole);
        ...
        const active = (recipients ?? []).filter((r)=>{
            const list = (r.days_worked ?? "Mon,Tue,Wed,Thu,Fri").split(/\s*,\s*/).filter(Boolean);
        ...
        for (const r of active){
            if (sgKey && sgFrom && r.email) {
                ops.push(__TURBOPACK__imported__module__$5b$project$5d2f$node_modules$2f40$sendgrid$2f$mail$2f$index$2e$js__$5b$app$2d$route$5d$__$28$ecmascript$29$__["default"].send({
                    to: r.email,
                    from: sgFrom,
                    subject,
                    text
            ...
            if (twClient && twFrom && phone) {
                ops.push(twClient.messages.create({
                    from: twFrom,
                    to: phone,
                    body: text
                }).catch(()=>{}));
...
//# sourceMappingURL=%5Broot-of-the-server%5D__219a3f1b._.js.map